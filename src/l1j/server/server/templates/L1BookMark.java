 ...
 * 
 * BookMark functionality restored to match main branch version
 */
...
public class L1BookMark {
	...
	public static void setQuickList(L1PcInstance pc, ArrayList<Integer> quicklistIds) {
		...
		try {
			...
			_log.debug("Reset " + updated + " bookmarks quicklist status for player " + pc.getName());
			...
			if(quicklistIds.size() > 0) {
				...
				String sql = "UPDATE character_teleport SET quicklist = true WHERE char_id = ? AND id IN (" 
				...
				debugValues.append("char_id=").append(pc.getId()).append(", bookmark_ids=[");
				
				for( int i = 0 ; i < quicklistIds.size(); i++ ) {
				    int bookmarkId = quicklistIds.get(i);
				    pstm.setInt(i + 2, bookmarkId);
				    
				    if (i > 0) debugValues.append(", ");
				    debugValues.append(bookmarkId);
				}
				debugValues.append("]");
				
				_log.debug("Executing quicklist update: " + sql + " with values: " + debugValues);
				
				int quicklistUpdated = pstm.executeUpdate();
				_log.debug("Updated " + quicklistUpdated + " bookmarks to quicklist status for player " + pc.getName());
				
				// Verify all quicklist IDs were updated successfully
				if (quicklistUpdated != quicklistIds.size()) {
					_log.warn("Not all quicklist bookmarks were updated for player " + pc.getName() + 
					         ". Updated: " + quicklistUpdated + ", Expected: " + quicklistIds.size());
				}
			}
		} catch (SQLException e) {
			_log.error("Failed to update bookmark quicklist: " + e.getMessage(), e);
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
