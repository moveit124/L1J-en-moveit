...
public class L1Skill {
	...
	private static final Logger _log = LoggerFactory.getLogger(L1Skill.class);
	...
	public L1SkillExecutor newExecutor() {
	    if (_impl == null || _impl.trim().isEmpty()) {
	        _log.error("❌ Skill executor class is missing for skillId=" + _skillId + ", name='" + _name + "'");
	        return null;
	    }
	    ...
	    try {
	        ...
	        _log.error("❌ Failed to load skill executor for skillId=" + _skillId + ", className='" + _impl + "'", e);
	        return null;
	    }
	}
