...
public class MapTimeController implements Runnable {
	...
	private void checkResetTime() {
		...
		cal.setTimeZone(TimeZone.getTimeZone(Config.TIME_ZONE)); // Use configured timezone
		...
		if (timer != null) {
			for(Area each : timer.getAreas()) {
				...
				_log.info("Resetting timer for area " + each.getAreaId() + " at " + String.format("%1$TH:%1$TM",cal) + " " + Config.TIME_ZONE);
