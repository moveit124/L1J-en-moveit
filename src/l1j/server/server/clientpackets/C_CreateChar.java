...
public class C_CreateChar extends ClientBasePacket {
	...
	private static void initNewChar(Client client, L1PcInstance pc)
		...
		// === Auto-mute new character if their IP is linked to a muted character ===
		Connection con = null;
		PreparedStatement ps1 = null, ps2 = null, insert = null;
		ResultSet rs1 = null, rs2 = null;

		try {
			con = L1DatabaseFactory.getInstance().getConnection();

			// Step 1: Get IP of this account
			ps1 = con.prepareStatement("SELECT ip FROM accounts WHERE login = ?");
			ps1.setString(1, pc.getAccountName());
			rs1 = ps1.executeQuery();

			if (rs1.next()) {
				String ip = rs1.getString("ip");

				// Step 2: Find any active mute linked to this IP
				ps2 = con.prepareStatement(
					"SELECT cm.end_time FROM character_mute cm " +
					"JOIN characters c ON cm.char_id = c.objid " +
					"JOIN accounts a ON c.account_name = a.login " +
					"WHERE a.ip = ? AND cm.end_time > CURRENT_TIMESTAMP " +
					"ORDER BY cm.end_time DESC LIMIT 1"
				);
				ps2.setString(1, ip);
				rs2 = ps2.executeQuery();

				if (rs2.next()) {
					Timestamp sharedEndTime = rs2.getTimestamp("end_time");

					// Step 3: Apply the same mute to this new character
					insert = con.prepareStatement("REPLACE INTO character_mute (char_id, end_time) VALUES (?, ?)");
					insert.setInt(1, pc.getId());
					insert.setTimestamp(2, sharedEndTime);
					insert.execute();

					// Apply the memory-only debuff capped to 40,000 seconds
					long remainingMs = sharedEndTime.getTime() - System.currentTimeMillis();
					int capped = (int) Math.min(remainingMs, 40_000_000L);

					pc.setSkillEffect(STATUS_CHAT_PROHIBITED, capped);
					pc.sendPackets(new S_ServerMessage(286, "You are muted due to shared IP with a banned character."));
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			SQLUtil.close(insert);
			SQLUtil.close(rs2);
			SQLUtil.close(ps2);
			SQLUtil.close(rs1);
			SQLUtil.close(ps1);
