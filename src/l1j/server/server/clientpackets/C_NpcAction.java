...
public class C_NpcAction extends ClientBasePacket {
	...
	public C_NpcAction(byte abyte0[], Client client) throws Exception {
		...
		// AFDFASGDAFDSFSDAF
		L1NpcInstance npc = (obj instanceof L1NpcInstance) ? (L1NpcInstance) obj : null;
		if (npc == null || npc.getNpcTemplate() == null) {
		    _log.warn("❌ NPCAction: obj is not an L1NpcInstance or npc template is null. ObjId=" + objid + ", Player=" + (pc != null ? pc.getName() : "null"));
		    return;
		}
		...
		if (s.equalsIgnoreCase("buy")) {
			...
			L1NpcInstance npc1 = (L1NpcInstance) obj;
			int npcId = npc1.getNpcTemplate().get_npcId();
			...
			if (isRent) {
				...
				pc.setHall(false);
				pc.sendPackets(new S_HowManyKey(npc1, 300, 1, 8, "inn2"));
			}
		}
			...
			if (pc.isCrown()) {
				...
				if (isRent) {
					...
					pc.setInnRoomNumber(roomNumber);
					pc.setHall(true);
					pc.sendPackets(new S_HowManyKey(npc1, 300, 1, 8, "inn12"));
				}
			} else {
				htmlid = "inn10";
			}
			...
			if (isBreak) {
				htmldata = new String[]  {npc1.getName(), String.valueOf(price)};
				htmlid = "inn20";
			...
			// 「はい。私がご協力しましょう」
				...
				if (pc.getInventory().checkItem(41007, 1)) { // イリスの命令書：霊魂の安息
					...
					String npcName1 = npc1.getNpcTemplate().get_name();
					String itemName = item.getItem().getName();
					pc.sendPackets(new S_ServerMessage(143, npcName1, itemName));
					htmlid = "eris6";
				}
			} else if (s.equalsIgnoreCase("C")) {
