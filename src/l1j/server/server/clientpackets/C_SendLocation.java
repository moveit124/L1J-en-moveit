...
public class C_SendLocation extends ClientBasePacket {
	...
	public C_SendLocation(byte abyte0[], Client client) {
		...
		if (type == 0x0b) {
			...
			// Log the bookmark update operation
			_log.debug("Player " + pc.getName() + " updating bookmarks. Total bookmarks: " + size);
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (index >= size) {
                    _log.warn("Player " + pc.getName() + " sent invalid bookmark index: " + index + 
                              ", max index is " + (size - 1));
                    continue;
                }
            ...
            for (int i = 0; i < 5; i++) {
            	...
            	if (index >= size) {
                    _log.warn("Player " + pc.getName() + " sent invalid quicklist index: " + index + 
                              ", max index is " + (size - 1));
                    continue;
                }
            	...
            	_log.debug("Player " + pc.getName() + " adding bookmark " + bookmark.getName() + 
            	           " (ID: " + bookmark.getId() + ") to quicklist");
            }
            
            // Log the final quicklist
            _log.debug("Player " + pc.getName() + " saved quicklist with " + 
                      quickLinks.size() + " bookmarks");
