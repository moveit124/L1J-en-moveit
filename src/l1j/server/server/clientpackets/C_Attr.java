				...
				if (c == 0) { // No
					...
					if (clan != null) {
						...
						if (Config.MAX_CLAN_MEMBER < 0) {
							// Original charisma-based logic
							int charisma = pc.getCha();
							boolean lv45quest = pc.getQuest().isEnd(L1Quest.QUEST_LEVEL45);

							if (pc.getLevel() >= 50) {
								maxMember = lv45quest ? charisma * 9 : charisma * 3;
							} else {
								maxMember = lv45quest ? charisma * 6 : charisma * 2;
							}
						} else if (Config.MAX_CLAN_MEMBER <= 10) {
							// If configured cap is 10 or less, skip scaling
							maxMember = Config.MAX_CLAN_MEMBER;
						} else {
							// Level-scaled logic
							int base = 10;
							int cap = Config.MAX_CLAN_MEMBER;
							int bonusMax = cap - base;
							int[] levels = { 50, 55, 60, 65, 70 };

							int bonusPerTier = bonusMax / levels.length;
							int remainder = bonusMax % levels.length;

							int levelBonus = 0;

							for (int i1 = 0; i1 < levels.length; i1++) {
								if (pc.getLevel() >= levels[i1]) {
									// Spread remainder across the later levels first
									levelBonus += bonusPerTier + (i1 >= levels.length - remainder ? 1 : 0);
								}
							}

							maxMember = base + levelBonus;
						}
						...
						if (joinPc.getClanid() == 0) {
							...
							int versionedId = clan.getClanId() + (clan.getEmblemVersion() << 16);
							joinPc.sendPackets(new S_ClanName(joinPc, versionedId, joinPc.getClanRank()));
							joinPc.save();
