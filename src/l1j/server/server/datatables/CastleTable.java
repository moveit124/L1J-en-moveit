	...
	public CastleTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				Object warTimeObj = rs.getObject(3);
				Timestamp warTime;

				if (warTimeObj instanceof Timestamp) {
				    warTime = (Timestamp) warTimeObj;
				} else if (warTimeObj instanceof LocalDateTime) {
				    warTime = Timestamp.valueOf((LocalDateTime) warTimeObj);
				} else {
				    warTime = new Timestamp(System.currentTimeMillis()); // Default fallback
				}

				castle.setWarTime(timestampToCalendar(warTime));
