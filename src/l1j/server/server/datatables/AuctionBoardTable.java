	...
	public AuctionBoardTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				java.time.LocalDateTime ldt = rs.getObject(4, java.time.LocalDateTime.class);
				Calendar cal = Calendar.getInstance();
				cal.setTime(java.util.Date.from(ldt.atZone(java.time.ZoneId.systemDefault()).toInstant()));
