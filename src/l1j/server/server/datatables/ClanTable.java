...
public class ClanTable {
	...
	private ClanTable() {
		{
			...
			try {
				...
				while (rs.next()) {
					...
					try {
						clan.setEmblemVersion(rs.getInt("emblemVersion")); // load version
					} catch (SQLException e) {
						clan.setEmblemVersion(1); // fallback in case old DB lacks column
					}
	...
	public L1Clan createClan(L1PcInstance player, String clan_name) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO clan_data SET clan_id=?, clan_name=?, leader_id=?, leader_name=?, hascastle=?, hashouse=?, emblemVersion=?");
			...
			pstm.setInt(7, clan.getEmblemVersion()); // usually 1
			pstm.execute();
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
		} finally {
	...
	public void updateClan(L1Clan clan) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE clan_data SET clan_id=?, leader_id=?, leader_name=?, hascastle=?, hashouse=?, emblemVersion=? WHERE clan_name=?");
			...
			pstm.setInt(6, clan.getEmblemVersion());
			pstm.setString(7, clan.getClanName());
			pstm.execute();
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
		} finally {
