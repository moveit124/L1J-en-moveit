...
public class ExcludeTable {
			...
			while (charIdRS.next()) {
				...
				try {
					...
					while (excludesRS.next()) {
						exclude.add(excludesRS.getString("exclude_name"));
					}
	...
	public void removeExclude(int charId, String excludeName) {
		...
		if (!exclude.contains(excludeName)) {
			return;
		}
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	/**
	 * Checks if a player has excluded another player
	 * 
	 * @param player the player who might have excluded someone
	 * @param target the player who might be excluded
	 * @return true if target is excluded by player
	 */
	public boolean isExcluded(L1PcInstance player, L1PcInstance target) {
		if (player == null || target == null) {
			return false;
		}
		
		L1ExcludingList excludeList = getExcludeList(player.getId());
		return excludeList.contains(target.getName());
	}
}
