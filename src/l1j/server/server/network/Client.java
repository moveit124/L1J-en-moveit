...
public class Client implements Runnable, PacketOutput {
	...
	private class LogoutDelay implements Runnable {
		...
		public void run() {
			try {
				if (_activeChar != null) {
					_activeChar.stopMapLimiter(); // ✅ Save timer before logout

					// Perform all game cleanup (inventory, buffs, pets, etc.)
	...
	public static void quitGame(L1PcInstance pc, String lastActiveChar) {
		...
		try {
			...
			_log.error("Error saving character data after quitting.", e);
		}
