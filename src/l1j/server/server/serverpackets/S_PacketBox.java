...
//	private static Logger _log = LoggerFactory.getLogger(S_PacketBox.class.getName());
	...
	public S_PacketBox(int subCode) {
		...
		case LOGOUT:
		case FISHING:
		    // Flag the packet specifically for fishing bobber animation
		    writeC(0x00); // This is needed for the client to properly identify the fishing bobber jump animation
		    break;
		case CALL_SOMETHING:
			callSomething();
			break;
	...
	private void callGfxEffect(int gfxid, int aid) {
		writeC(Opcodes.S_OPCODE_PACKETBOX);
		writeC(0x16);
		writeD(gfxid);
		writeD(aid);
	...
	private void sendDungeonTime() {
		Calendar cal = Calendar.getInstance(TimeZone.getTimeZone(Config.TIME_ZONE));
		writeC(Opcodes.S_OPCODE_PACKETBOX);
		writeC(0x40);
		writeS(Config.TIME_ZONE); // Time Zone
		writeC(cal.get(Calendar.MONTH) + 1);
		writeC(cal.get(Calendar.DATE));
		writeC(cal.get(Calendar.HOUR_OF_DAY));
		writeC(cal.get(Calendar.MINUTE));
	}
