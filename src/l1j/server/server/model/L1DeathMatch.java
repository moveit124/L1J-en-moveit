...
public class L1DeathMatch {
	...
	private static int gameStartMinPlayer = 1;
	private static int orderEntMinPlayer = 1;
	...
	private static volatile L1DeathMatch instance;

	public static L1DeathMatch getInstance() {
		if (instance == null) {
			synchronized (L1DeathMatch.class) {
				if (instance == null) {
	...
	private L1DoorInstance _doorLeft = null;
	...
	private L1DeathMatch() {
		try {
			...
			if (leftGfx != null) {
				...
				if (_doorLeft == null) {
					_log.error("Failed to create door for DeathMatch");
				}
			} else {
				_log.error("Could not find door gfx 6692 for DeathMatch");
			}
		} catch (Exception e) {
			_log.error("Error initializing DeathMatch", e);
		}
	}
	...
	public class CloseTimer extends TimerTask {
		...
		@Override
		public void run() {
			if (_door != null && _door.getOpenStatus() == ActionCodes.ACTION_Open) {
				for (L1Object obj : L1World.getInstance().getVisiblePoint(
