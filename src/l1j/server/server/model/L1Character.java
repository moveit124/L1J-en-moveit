...
public class L1Character extends L1Object {
	...
	public boolean glanceCheck(int tx, int ty) {
		...
		if (losCheck(getX(), getY(), tx, ty)) {
			return true;
		}
		// If that fails, try reverse line of sight
		if (losCheck(tx, ty, getX(), getY())) {
			return true;
		}
		return false;
	}
	...
	private boolean losCheck(int chx, int chy, int tx, int ty) {
	    for (int i = 0; i < 15; i++) { // Maximum range of 15 tiles (same as JP codebase)
	        ...
	        if (!getMap().isArrowPassable(chx, chy, losTargetDirection(chx, chy, tx, ty))) {
	            return false; // Failed because of obstacle
	        }
	    ...
	    return false; // Failed to reach target within 15 tiles
	}
