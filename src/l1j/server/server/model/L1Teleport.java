...
public class L1Teleport {
	private static Logger _log = LoggerFactory.getLogger(L1Teleport.class);
	...
	private static boolean isInvalidTile(L1Map map, int x, int y) {
		// Check if tile is water (value 0), cave wall (32) or has impassable flag set
		int tile = map.getTile(x, y);
		return tile == 0 || tile == 32 || !map.isPassable(x, y);
	}
