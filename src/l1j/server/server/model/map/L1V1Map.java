...
public class L1V1Map extends L1Map {
	...
	@Override
	public boolean isPassablePC(Point pt) {
		return isPassablePC(pt.getX(), pt.getY());
	}

	@Override
	public boolean isPassablePC(int x, int y) {
		return isPassablePC(x, y - 1, 4) || isPassablePC(x + 1, y, 6)
				|| isPassablePC(x, y + 1, 0) || isPassablePC(x - 1, y, 2);
	}

	@Override
	public boolean isPassablePC(Point pt, int heading) {
		return isPassablePC(pt.getX(), pt.getY(), heading);
	}
	...
	public boolean isPassable(int x, int y, int heading) {
	    ...
	    // If tile2 is a fence, block movement
	    if (tile2 == 1 || tile2 == 9 || tile2 == 21 || tile2 == 26 || tile2 == 28 || tile2 == 32 || 
	    		tile2 == 45 || tile2 == 46 ||   tile2 == 65 || tile2 == 73 ) {
	        return false;
	    }
	...
	@Override
    public boolean isPassablePC(int x, int y, int heading) {
        int tile1 = accessTile(x, y);
        int tile2;
