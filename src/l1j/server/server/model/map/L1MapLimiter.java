...
public class L1MapLimiter implements Runnable {
	...
	private MapTimerTable _mapTimerTable = null;
				...
				if (effect.getTime() > 0) {
					...
					if (enterTime <= effect.getDelay()) {
						_pc.sendPackets(new S_ServerMessage(1528, String.valueOf(enterTime)));
							...
							|| ((enterTime <= 600) && (enterTime % 60 == 0))) { // Every minute under 10 minutes
						if (h > 0) {
							if (m > 0) {
					...
					// Decrement and update time
					enterTime -= 1;
					_mapTimerTable.setEnterTime(enterTime);

					// ✅ Auto-save every 5 minutes
					if (enterTime % 300 == 0) {
						_mapTimerTable.save();
						// Optional debug:
						// System.out.println("[DEBUG] Auto-saved timer for " + _pc.getName() + " at " + enterTime + "s left.");
					}
				}
			} else {
	...
	private void teleport() {
		...
		// 🚫 Stop the limiter after teleport
		_pc.stopMapLimiter();
	}
	...
	public L1MapLimiter cloneForPlayer(L1PcInstance pc) {
		L1MapLimiter clone = new L1MapLimiter();
		clone._mapId = this._mapId;
		clone._areaId = this._areaId;
		clone._effect = this._effect;
		clone._pc = pc;
		clone._mapTimerTable = MapTimerTable.find(pc.getId(), this._areaId);

		if (clone._mapTimerTable == null) {
			clone._mapTimerTable = new MapTimerTable(pc.getId(), this._mapId, this._areaId, this._effect.getTime());
		}
