...
public class L1HauntedHouse {
	...
	private void startHauntedHouse() {
	    ...
	    for (L1PcInstance pc : _members) {
	        // ✅ Fully cancel buffs before morphing
	        try {
	        	L1Cancel.fullCancel(pc);
	        } catch (Exception e) {
	            _log.error("Failed to cancel player buffs for Haunted House", e);
	        }
	...
	public void endHauntedHouse() {
		...
		for (L1PcInstance pc : _members) {
		    ...
		    // 🛡️ Add safe increment here
		    BotCheckActivityManager.incrementHauntedHouse(pc);
		    pc.addHauntedHouseEntry();
