...
import static l1j.server.server.model.item.L1ItemId.ELINDAIRE_STAFF;
...
public class L1Attack {
	...
	public L1Attack(L1Character attacker, L1Character target, int skillId) {
		...
		_target = target;
		if (_target == null) {
		    _targetId = -1; // or 0, or whatever is safe in your logic
		    LoggerFactory.getLogger(L1Attack.class).warn("❌ L1Attack constructed with null target. Attacker: {}",
		        attacker != null ? attacker.getName() + " [ID: " + attacker.getId() + "]" : "null");
		    return;
		}
	...
	public int getWeightHitModifier(final L1PcInstance pc) {
	    return 1; // Temporary fix to prevent infinite recursion
	}
		...
		if (_npc.getNpcTemplate().get_poisonatk() != 0) {
			if (12 >= ThreadLocalRandom.current().nextInt(100) + 1) { // 12% chance of poison
				...
				if (_npc.getNpcTemplate().get_poisonatk() == 1) { // normal
					...
					L1ParalysisPoison.doInfection(target, 20000, 18000);
				}
			}
...
//		}
	...
	private void actionPc() {
		...
		// BEGIN: Clamp arrow target coordinates to valid map bounds
		L1Map map = _pc.getMap();
		int mapX1 = map.getX();
		int mapX2 = mapX1 + map.getWidth();
		int mapY1 = map.getY();
		int mapY2 = mapY1 + map.getHeight();
		_targetX = Math.max(mapX1, Math.min(_targetX, mapX2));
		_targetY = Math.max(mapY1, Math.min(_targetY, mapY2));
		// END: Clamp arrow target coordinates
		if (!_pc.glanceCheck(_targetX, _targetY)) {
			System.out.println("DEBUG: glanceCheck failed for target (" + _targetX + ", " + _targetY + "). Map.isPassable: " + map.isArrowPassable(_targetX, _targetY));
		}
		if (isBow) {
			if (_arrow != null) {
	...
	// Inside L1Attack.java, modify commit() method:
	public void commit() {
	...
	private void commitNpc() {
	    if (_calcType == PC_NPC) {
	        ...
	        // ✅ Add Debug Logs Before Damage Application
	        if (_damage > 0 && _targetNpc instanceof L1MonsterInstance) {
	            ((L1MonsterInstance) _targetNpc).addLastAttackedBy(_pc);
	        }
