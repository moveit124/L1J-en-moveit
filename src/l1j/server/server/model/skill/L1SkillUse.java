...
public class L1SkillUse {
	...
	private static final int[] CAST_WITH_INVIS = { 1, 2, 3, 5, 8, 9, 12, 13, 14, 19, 21, 26, 31, 32, 35, 37, 42, 43, 44,
			...
			163, 164, 165, 166, 168, 169, 170, 171, 194, 213, SOUL_OF_FLAME, ADDITIONAL_FIRE, DRAGON_SKIN, AWAKEN_ANTHARAS,
	...
	private static final int[] NON_PINK_SKILLS = { DETECTION, 194, 213  };
...
//		}
	...
	private boolean isNormalSkillUsable() {
		if (_user instanceof L1PcInstance) {
			...
			if (pc.getInventory().getWeight240() >= 197) {
				pc.sendPackets(new S_SystemMessage("You cannot use the spell while so heavily loaded with baggage.")); // You cannot use the spell while so heavily loaded with
															// baggage.
				return false;
			}
			...
			if (poly != null && !poly.canUseSkill()) {
				pc.sendPackets(new S_SystemMessage("You can't cast a spell in that state.")); // You can't cast a spell in that state.				
				return false;
			}
			...
			if (_skillId == ELEMENTAL_PROTECTION && pc.getElfAttr() == 0) {
				pc.sendPackets(new S_SystemMessage("Magic has failed.")); // Magic has failed.
				return false;
			}
			...
			if (pc.isSkillDelay()) {
				pc.sendPackets(new S_SystemMessage("That spell is on cooldown.")); //
				return false;
			}
			...
			if (_skillId == DISINTEGRATE && pc.getLawful() < 500) {
				pc.sendPackets(new S_SystemMessage("Your  alignment must be 'Lawful' to cast this spell.")); // Your alignment must be 'Lawful' to cast this spell.
				return false;
			}
			...
			if (_skillId == CUBE_IGNITION || _skillId == CUBE_QUAKE || _skillId == CUBE_SHOCK
				...
				if (isNearSameCube) {
					pc.sendPackets(new S_SystemMessage("The cube is already summoned on the ground.")); // The cube is already summoned on the ground.
					return false;
				}
			}
			...
			if (isItemConsume() == false && !_player.isGm()) {
				pc.sendPackets(new S_SystemMessage("There are not enough ingredients to use the spell.")); // There are not enough ingredients to use the spell.
				return false;
			}
		} else if (_user instanceof L1NpcInstance) {
		...
		try {
			...
			if (cha instanceof L1PcInstance) {
				...
				if (pc.hasSkillEffect(ABSOLUTE_BARRIER)) {
					if (_skillId == CURSE_BLIND || _skillId == WEAPON_BREAK || _skillId == DARKNESS
							...
							|| _skillId == IMMUNE_TO_HARM || _skillId == REMOVE_CURSE || _skillId == ARM_BREAKER
							|| _skillId == 194 || _skillId == 213) {
						return true;
					} else {
						return false;
					}
			...
			if (cha instanceof L1NpcInstance) {
				...
				if (hiddenStatus == L1NpcInstance.HIDDEN_STATUS_SINK) {
					if (_skillId == DETECTION || _skillId == COUNTER_DETECTION || _skillId == ARM_BREAKER || _skillId == 194 || _skillId == 213) {
						return true;
					} else {
						return false;
					}
	...
	private boolean isHPMPConsume() {
		...
		if (_user instanceof L1NpcInstance) {
			...
			if ((_skillId == ENERGY_BOLT) && inventory.checkEquipped(20012)) {
				_mpConsume = 1;
			}
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (_targetList.size() == 0 && !(_skill.getTarget().equals("none"))) {
				...
				if (isSkillAction && !_skipCastingAnimation) { // Skip casting animation if requested
			...
			if (_skill.getTarget().equals("attack") && _skillId != 18) {
				...
				if (_skillId != 5 && _skillId != 69 && _skillId != 131) {
					if (isSkillAction && _skillId != BURNING_SLASH && !_skipCastingAnimation) { // Skip casting animation if requested
					...
					if (_skillId == COUNTER_MAGIC || _skillId == COUNTER_BARRIER || _skillId == COUNTER_MIRROR) {
						...
						if (_skillId == UNCANNY_DODGE) {
							...
							_player.addDodge((byte) 5);
							_player.sendPackets(new S_PacketBox(S_PacketBox.DODGE_RATE_PLUS, _player.getDodge()));
						}
		...
		for (int[] skills : NONOVERLAPPING_SKILLS) {
			for (int id : skills) {
				if (id == _skillId) {
					...
					return; // ✅ prevents stopping the current skill after applying it
				}
			}
		}
	}
...
private void runSkill() {
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				if (_skillId == HASTE) {
				...
				} else if (_skillId == DETECTION || _skillId == ARM_BREAKER || _skillId == 194 || _skillId == 213) {
					if (cha instanceof L1NpcInstance) {
						L1NpcInstance npc = (L1NpcInstance) cha;
					...
					int[] BowGFX = { 
						138, 37, 3860, 3126, 3420, 2284, 3105, 3145, 3148, 3151, 
						3871, 4125, 2323, 3892, 3895, 3898, 3901, 4917, 4918, 4919, 
						4950, 6087, 6140, 6145, 6150, 6155, 6160, 6269, 6272, 6275, 
						6278, 6826, 6827, 6836, 6837, 6846, 6847, 6856, 6857, 6866, 
						6867, 6876, 6877, 6886, 6887, 
						8860, 8786, 8792, 8798, 8804, 8808, 8842, 8845
					};
				...
				if (_calcType == PC_PC || _calcType == NPC_PC) {
					...
					if (_skillId == GLOWING_AURA) {
						...
						if (pc.getInventory().consumeItem(L1ItemId.FORBIDDEN_FRUIT, 1)) {
							...
							pc.sendPackets(new S_SystemMessage("You feel like your blood is boiling deep inside your body."));
						} else {
	...
	/**
	 * Set whether to skip the casting animation but still show spell effects
	 * @param skip True to skip the casting animation, false to show it
	 */
