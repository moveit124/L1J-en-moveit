...
//		}
	...
	public void setDigestItem(L1ItemInstance item) {
		_digestItems.put(Integer.valueOf(item.getId()), Integer.valueOf(getNpcTemplate().get_digestitem()));
	...
	public int moveDirection(int x, int y, double d) {
	    ...
	    // **New: Verify passability before NPC moves**
	    if (dir != -1) {
	        int nx = getX();
	        int ny = getY();
	        ...
	        if (!getMap().isPassable(nx, ny)) {
	            // Removed debug logging: "NPC blocked at (x,y) on tile: X"
	            return -1; // Prevent movement into impassable tiles
	        }
	    }
