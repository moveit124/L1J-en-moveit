...
public class L1MonsterInstance extends L1NpcInstance {
	...
	public boolean isBoss() {
	    return L1BossSpawn.isBoss(getNpcId());
	}
	// In L1MonsterInstance.java (Add this inside the class)

	private Set<String> lastAttackers = new HashSet<>();
	private long lastAttackTime = 0;

	// Called when the monster takes damage
	public void addLastAttackedBy(L1PcInstance player) {
	    if (player == null) return;
	    lastAttackers.add(player.getName());
	    lastAttackTime = System.currentTimeMillis();
	}

	// Get the last player who attacked
	public boolean hasAttacked(L1PcInstance player) {
	    if (player == null) return false;
	    return lastAttackers.contains(player.getName());
	}

	public void removeLastAttackedBy(L1PcInstance player) {
	    if (player == null) return;
	    lastAttackers.remove(player.getName());
	}


	public boolean wasAttackedRecently() {
	    return !lastAttackers.isEmpty() && (System.currentTimeMillis() - lastAttackTime) <= 60000;
	}

	private transient int _aiRetryCount = 0;
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		...
		if (0 < getCurrentHp()) {
			...
			// Retry AI if it's not running, up to 3 times
			if (!isAiRunning()) {
				_aiRetryCount = 0;
				for (int i = 1; i <= 3; i++) {
					final int retryDelay = (int) Math.pow(2, i - 1) * 1000; // 1000, 2000, 4000
					GeneralThreadPool.getInstance().schedule(() -> {
						if (!isAiRunning() && _aiRetryCount < 3) {
							onNpcAI();
							_aiRetryCount++;
						}
					}, retryDelay);
	...
	public void receiveDamage(L1Character attacker, int damage) {
	    ...
	    //System.out.println("[DEBUG] Monster " + getName() + " HP BEFORE: " + getCurrentHp() + "/" + getMaxHp() + " | Damage Taken: " + damage);
	...
	private void distributeExpDropKarma(L1Character lastAttacker) {
		...
		if (pc != null) {
			...
			if (isDead()) {
				distributeDrop();
				giveKarma(pc);
				if (pc.isRecentlyUsedAoeSkill()) {
				    pc.addMonsterKillBuffer(0.5); // or 0.5 if you want
				} else {
