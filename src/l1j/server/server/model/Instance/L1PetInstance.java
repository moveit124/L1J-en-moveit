...
public class L1PetInstance extends L1NpcInstance {
	...
	public synchronized void death(L1Character lastAttacker) {
		if (!isDead()) {
			...
			setCurrentHp(0);
			// If player killed the pet, reduce their lawfulness
			if (lastAttacker instanceof L1PcInstance) {
			    L1PcInstance killer = (L1PcInstance) lastAttacker;
			    if (killer.getZoneType() == ZoneType.Combat) {
			        return;
			    }
			    int currentLaw = killer.getLawful();
			    int loss;

			    if (currentLaw >= -10000) {
			        double norm = Math.max(0.0, Math.min(1.0, (currentLaw + 10000) / 30000.0));
			        double curved = 1.0 - Math.pow(1.0 - norm, 2.2);
			        double rawLoss = 1000 + curved * 7000;
			        loss = (int) rawLoss;
			        loss = Math.min(loss, 8000);
			    } else {
			        loss = 1000;
			    }

			    int newLaw = currentLaw - loss;
			    if (newLaw < -32768) {
			        newLaw = -32768;
			    }
			    int actualLoss = currentLaw - newLaw;
			    System.out.println("Law before: " + currentLaw + ", Law after: " + newLaw + ", Loss: " + actualLoss);

			    killer.setLawful(newLaw);

			    // Pet morality message progression
			    String message;
			    if (currentLaw >= 10000) {
			        message = "Your soul is tainted by unnecessary cruelty.";
			    } else if (currentLaw >= 5000) {
			        message = "You feel nothing as the creature falls.";
			    } else if (currentLaw >= 0) {
			        message = "Compassion fades... replaced by silence. It was just an animal.";
			    } else if (currentLaw >= -10000) {
			        message = "The creature’s whimper satisfied something in you.";
			    } else {
			        message = "You watched it die... and smiled.";
			    }

			    killer.sendPackets(new S_SystemMessage(message));
			}
	...
	private boolean isFood(L1Item item) {
		...
		if (petId == 45313 || petId == 45710) { // Tiger, Battle Tiger
			if (itemId == 50572 || itemId == 50574 || itemId == 40057) {
				result = true;
			}
		} else if (petId == 45711 || petId == 45712) { // Jindo puppy
			if (itemId == 50573 || itemId == 50575 || itemId == 40057) {
				result = true;
			}
		} else { // All other pets
