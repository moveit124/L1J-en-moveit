...
public class L1DollInstance extends L1NpcInstance {
	...
	public static final int DOLLTYPE_GIANT = 9;
	public static final int DOLLTYPE_CYCLOPS = 10;
	public static final int DOLLTYPE_MERMAID = 11;
	public static final int DOLLTYPE_BRUTE = 12;
...
//	private ScheduledFuture<?> _dollFuture;
	...
	public void deleteDoll() {
		...
		((L1PcInstance) _master).addStr(-getStrBonusByDoll());
		((L1PcInstance) _master).addDamageReductionByDoll(-getDamageReductionByDoll());
		((L1PcInstance) _master).addHpRegeneration(-getHpRegenBonusByDoll());
		((L1PcInstance) _master).addResistAll(-getResistAllByDoll());
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		...
		// Apply doll effects when spawned
		if (_master instanceof L1PcInstance) {
			L1PcInstance pc = (L1PcInstance) _master;
			pc.addStr(getStrBonusByDoll());
			pc.addDamageReductionByDoll(getDamageReductionByDoll());
			pc.addHpRegeneration(getHpRegenBonusByDoll());
			pc.addResistAll(getResistAllByDoll());
		}
	}
	...
	public int getDamageReductionByDoll() {
		...
		} else if (dollType == DOLLTYPE_BRUTE) {
			damageReduction = 2;
		}
	...
	public int getStrBonusByDoll() {
		int strBonus = 0;
		int dollType = getDollType();
		if (dollType == DOLLTYPE_GIANT) {
			strBonus = 1;
		}
		return strBonus;
	}

	public int getHpRegenBonusByDoll() {
		int hpRegenBonus = 0;
		int dollType = getDollType();
		if (dollType == DOLLTYPE_CYCLOPS) {
			hpRegenBonus = 3;
		}
		return hpRegenBonus;
	}

	public int getResistAllByDoll() {
		int resistAll = 0;
		int dollType = getDollType();
		if (dollType == DOLLTYPE_MERMAID) {
			resistAll = 3;
		}
		return resistAll;
	}
