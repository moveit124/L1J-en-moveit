...
public class L1Shop {
    ...
    private L1ShopItem getPurchasingItem(int itemId) {
        for (L1ShopItem shopItem : _purchasingItems) {
            if (shopItem.getItemId() == itemId) {
                if (shopItem.getItem() == null) { // Debug
                    System.err.println("Error: getPurchasingItem() found null L1Item for itemId: " + itemId);
                }
                return shopItem;
            }
        }
    ...
    public L1AssessedItem assessItem(L1ItemInstance item) {
        ...
        // Use the new getAssessedPrice() method from L1ShopItem
        return new L1AssessedItem(item.getId(), shopItem.getAssessedPrice());
    }

    // Removed the old private getAssessedPrice(L1ShopItem item) method
    ...
    public List<L1AssessedItem> assessItems(L1PcInventory inv) {
        ...
        for (L1ShopItem item : _purchasingItems) {
            for (L1ItemInstance targetItem : inv.findItemsId(item.getItemId())) {
                ...
                // Now call the new method directly from L1ShopItem
                result.add(new L1AssessedItem(targetItem.getId(), item.getAssessedPrice()));
            }
        }
        return result;
    }
 ...
 */
	//Only Heine gets taxes from ALL towns
	private void payCastleTax(L1ShopBuyOrderList orderList) {
		L1TaxCalculator calc = orderList.getTaxCalculator();

		// Calculate total castle tax without subtracting national/diad
		int fullTax = calc.calcCastleTaxPrice(orderList, false); // false = don't subtract national/diad

		// Route all tax to Heine (castle_id = 5)
		final int HEINE_CASTLE_ID = 5;
		L1Castle heine = CastleTable.getInstance().getCastleTable(HEINE_CASTLE_ID);

		if (fullTax > 0 && heine != null) {
			synchronized (heine) {
				int money = heine.getPublicMoney();
				if (money < 2_000_000_000) {
					money += fullTax;
					heine.setPublicMoney(money);
					CastleTable.getInstance().updateCastle(heine);
				}
			}
		}
	}
