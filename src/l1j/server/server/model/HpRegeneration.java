...
public class HpRegeneration implements Runnable {
	...
	public void run() {
		try {
			if (_pc.isDead() || _pc.getNetConnection() == null || Thread.interrupted()) {
			    return;
			}
	...
	public void regenHp() {
		...
		if (_pc.get_food() < 3 || isOverWeight(_pc)
			...
			bonus = 0;
			conHp = 0; // Disable Constitution regen when overweight
			if (equipHpr > 0) {
				equipHpr = 0;
			}
		}
		...
		if (regen_state == 1) { //Attacking
			newHp += ((bonus + equipHpr) + (conHp / 2)) * 0.6;
		} else if (regen_state == 2) { //Moving
			newHp += ((bonus + equipHpr) + (conHp / 2)) * 0.8;
		} else { //Standing Still
			newHp += (bonus + equipHpr) + (conHp / 2);
		}
		if (newHp < 1) {
			newHp = 1;
		}
