...
public class Shutdown extends Thread {
	...
	private void saveData() {
		try {
			_log.info("Server shutdown procedure started.");
			GameServer.getInstance().disconnectAllCharacters();

			// Shutdown the dedicated DB logout executor
			try {
				_log.info("Shutting down DB Logout Executor...");
				shutdownExecutorService(GameServer.DB_LOGOUT_EXECUTOR, "DB Logout Executor", 15);
			} catch (Exception e) {
				_log.error("Error during DB Logout Executor shutdown", e);
			}

			// Shut down the general thread pool services gracefully
			_log.info("Shutting down General Thread Pool services...");
			try {
				// Access the private fields via a helper method if needed, or reflection (less ideal)
				// Assuming GeneralThreadPool provides accessors or a shutdown method:
				GeneralThreadPool gtp = GeneralThreadPool.getInstance();
				// We need methods in GeneralThreadPool to access/shutdown its executors.
				// Let's assume placeholder methods exist for now:
				// shutdownExecutorService(gtp.getExecutor(), "General Executor", 15);
				// shutdownExecutorService(gtp.getScheduler(), "General Scheduler", 15);
				// shutdownExecutorService(gtp.getPcScheduler(), "PC Scheduler", 15);
				_log.warn("TODO: Implement proper shutdown for executors within GeneralThreadPool.");
				// For now, just log that it needs implementation.

			} catch (Exception e) {
				_log.error("Error during General Thread Pool shutdown", e);
			}
			_log.info("Server shutdown save data finished.");
		} catch (Exception e) {
			_log.error("Critical error during saveData() shutdown hook.", e);
		}
	}

	/**
	 * Helper method to shut down an ExecutorService gracefully.
	 * @param service The ExecutorService to shut down.
	 * @param serviceName A descriptive name for logging.
	 * @param timeoutSeconds The timeout in seconds to wait for termination.
	 */
	private void shutdownExecutorService(ExecutorService service, String serviceName, int timeoutSeconds) {
		if (service == null) return;
		try {
			service.shutdown();
			if (!service.awaitTermination(timeoutSeconds, TimeUnit.SECONDS)) {
				_log.warn(serviceName + " did not terminate cleanly after " + timeoutSeconds + " seconds. Forcing shutdown...");
				service.shutdownNow();
				if (!service.awaitTermination(timeoutSeconds, TimeUnit.SECONDS)) {
					 _log.error(serviceName + " did not terminate even after forced shutdown.");
				}
			}
			_log.info(serviceName + " finished shutdown.");
		} catch (InterruptedException ie) {
			_log.warn("Shutdown interrupted while waiting for " + serviceName + ". Forcing shutdown...");
			service.shutdownNow();
			Thread.currentThread().interrupt(); // Preserve interrupt status
		} catch (Exception e) {
			_log.error("Error during " + serviceName + " shutdown", e);
		}
	}

	// Add overload for ScheduledExecutorService if needed, or cast
	private void shutdownExecutorService(ScheduledExecutorService service, String serviceName, int timeoutSeconds) {
		shutdownExecutorService((ExecutorService) service, serviceName, timeoutSeconds);
	}
